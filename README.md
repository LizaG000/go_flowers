...
В проекте RabbitMQ используется для передачи запросов между клиентом и серверной частью интернет-магазина цветов.
...
Клиент отправляет запрос на создание цветка в очередь api.requests. Сервер получает сообщение, создаёт запись в PostgreSQL и отправляет ответ в очередь api.responses.

Очереди
| Очередь         | Назначение                   |
| --------------- | ---------------------------- |
| `api.requests`  | Запросы от клиента к серверу |
| `api.responses` | Ответы сервера клиенту       |
...
Поле correlation_id совпадает с id исходного запроса и позволяет клиенту определить, к какому запросу относится ответ.

Идемпотентность

Каждый запрос содержит idempotency_key. Сервер проверяет этот ключ в таблице idempotency_keys.

Если ключ уже существует, сервер возвращает сохранённый результат. Если ключ новый, создаётся цветок и результат обработки сохраняется. Это предотвращает создание дубликатов при повторной отправке сообщения.

Безопасность

JWT-токен передаётся в поле auth:
Bearer <jwt-token>
